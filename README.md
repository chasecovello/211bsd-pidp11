...
It's time to edit configuration files. If you're a vi user, this is a great opportunity to use the original BSD `vi`. It's not as fancy as vim but it works fine. If you like emacs, `jove` is an emacs clone that comes installed.
...
### Set the correct number of rows for your terminal

You'll need to manually set the number of terminal rows in each user's shell startup files. It's not automatic like on modern systems. For root, `sh` uses `/.profile`, and `cshrc` uses `/.login`. For other users, those files are in their home directory. Replace the `24` in `stty rows 24` with the correct number for your terminal.

### Set up networking

You will need to edit `/etc/hosts`, `/etc/netstart`, and `/etc/resolv.conf` with the correct IP addresses. You can also change the hostname if you want.
...
You can see the web server with CGI in action (assuming my PiDP-11 is up and running 2.11BSD) at <http://chasecovello.ddns.net/>.

### Set up printing

You can set up a print server on Raspberry Pi OS (or anywhere else, really), and then configure 2.11BSD `lpd` to send print jobs to the remote `lpd` server.
...
2. Set options in your new config file as needed
3. `./config NEWCONFIGNAME`
4. `cd /usr/src/sys/NEWCONFIGNAME ; make`
